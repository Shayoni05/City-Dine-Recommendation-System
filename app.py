...
def predict_restaurant(location, cuisine, budget, city):
    ...
    predicted_rating = rating_model.predict([[location_encoded, cuisine_encoded, budget_encoded, city_encoded]])[0]
    
    # Find the closest restaurant with the predicted rating
    closest_index = (df['rating'] - predicted_rating).abs().idxmin()
    closest_restaurant = df.loc[closest_index, 'restaurants']
    
    return label_encoders['restaurants'].inverse_transform([closest_restaurant])[0], predicted_rating
